...
export default function ProjectCard({ title, description, demoLink, image }: ProjectCardProps) {
  ...
  return (
    <div
      ref={cardRef}
      className="bg-white/10 rounded-xl shadow-md text-center hover:-translate-y-1 transition-all duration-200 hover:shadow-lg border-white/10 backdrop-blur-2xl relative overflow-hidden group"
    ...
    >
      ...
      <div className="p-6 relative z-10">
        <h3 className="text-xl font-bold text-white mb-2 relative overflow-visible">
          {renderAnimatedTitle(title)}
          <span className="absolute left-1/2 -translate-x-1/2 bottom-1 w-8 h-1 bg-gray-200 rounded-full opacity-0 group-hover:opacity-80 transition-all duration-300 group-hover:bottom-[-2px]"></span>
        </h3>
